...
int main (int argc, char *argv[])
{
    ...
    MPI_Comm_split(MPI_COMM_WORLD, old_rank / GROUP_SIZE, old_rank % GROUP_SIZE,
                   &custom_group);
    ...
    // Receive the rank.
    if (new_rank == 0) {
        MPI_Recv(&recv_rank, 1, MPI_INT, GROUP_SIZE - 1, MPI_ANY_TAG,
                 custom_group, MPI_STATUS_IGNORE);
    } else {
        MPI_Recv(&recv_rank, 1, MPI_INT, new_rank - 1, MPI_ANY_TAG,
                 custom_group, MPI_STATUS_IGNORE);
    }
